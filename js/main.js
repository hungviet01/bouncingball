class Game {
  ...
  constructor() {
    ...
    this.colors = ["#FF4571", "#FFD145", "#4c46ff"]; // the 3 colors used in the game
...
class Color {
  ...
  constructor() {
    this.colors = ["#FF4571", "#FFD145", "#4c46ff"];
  ...
  colorcodeToName(color) {
    let colors = ["#FF4571", "#FFD145", "#4c46ff"];
  ...
  static getColorFromClass(el) {
    ...
    for (var i = 0, len = classes.length; i < len; i++) {
      if (classes[i] == 'red' || classes[i] == 'yellow' || classes[i] == 'blue') {
        return classes[i];
      }
    }
  }}
